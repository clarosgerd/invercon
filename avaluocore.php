...
$avaluocore_php = NULL; // Initialize page object first

class cavaluocore_php {
	...
	// Table name
	var $TableName = 'avaluocore.php';

	// Page object name
	var $PageObjName = 'avaluocore_php';
	...
	function __construct() {
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'avaluocore.php', TRUE);
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("custom", "avaluocore_php", $url, "", "avaluocore_php", TRUE);
		$this->Heading = $Language->TablePhrase("avaluocore_php", "TblCaption"); 
	}
}
?>
<?php ew_Header(FALSE) ?>
...
// Create page object
if (!isset($avaluocore_php)) $avaluocore_php = new cavaluocore_php();

// Page init
$avaluocore_php->Page_Init();

// Page main
$avaluocore_php->Page_Main();
...
//echo $_GET['type'];
//$_GET['case']case=of
if (isset($_GET['id']) || isset($_GET['type']) || isset($_GET['case']) || isset($_GET['job'])) {

if ($_GET['case']=="of") ////if of
{
...
//echo $TheQuery;
	...
	if ($all_mensaje_result && $all_mensaje_result->RecordCount() > 0) {
		...
		/** @var TYPE_NAME $this */
		$avaluocore_php->setSuccessMessage($sData);
		//ew_SendEmail()
		...
		$avaluocore_php->setFailureMessage("No records found.");
		header('Location: viewsolicitudedit.php?showdetail=&id='.$_GET['id']);
	}
}
//email
...
//echo $TheQuery;
	...
	if ($all_mensaje_result && $all_mensaje_result->RecordCount() > 0) {
		...
		$MyResult = ew_Execute($sql_new);
			$avaluocore_php->setSuccessMessage("Correo enviado");
		}
		else{
			$avaluocore_php->setFailureMessage("Correo no enviado");
		}
	//	var_dump($bEmailSent);
		//$avaluocore_php->s
...
//echo $TheQuery;
	...
	if ($all_mensaje_result && $all_mensaje_result->RecordCount() > 0) {
		...
		if (isset($email_envio)) {
			...
			$MyResult = ew_Execute($sql_new);
			$avaluocore_php->setSuccessMessage("Notificacion enviada");
			header('Location: viewsolicitudedit.php?showdetail=&id=' . $_GET['id']);
			exit;
		}else{

			$avaluocore_php->setFailureMessage("Notificacion no enviada");
			header('Location: viewsolicitudedit.php?showdetail=&id=' . $_GET['id']);
			exit;
		}
	} else {
...
if (isset($_GET['id']))
{
	...
	ew_Execute($TheQuery);
	$avaluocore_php->setSuccessMessage("Notificacion Leida");
