...
$offline_messages_list = NULL; // Initialize page object first

class coffline_messages_list extends coffline_messages {
	...
	// Page object name
	var $PageObjName = 'offline_messages_list';

	// Grid form hidden field names
	var $FormName = 'foffline_messageslist';
	...
	function __construct() {
		...
		$this->AddUrl = "offline_messagesadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "offline_messagesdelete.php";
		$this->MultiUpdateUrl = "offline_messagesupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption foffline_messageslistsrch";
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.foffline_messageslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_offline_messages\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_offline_messages',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.foffline_messageslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($offline_messages_list)) $offline_messages_list = new coffline_messages_list();

// Page init
$offline_messages_list->Page_Init();

// Page main
$offline_messages_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$offline_messages_list->Page_Render();
?>
...
var CurrentForm = foffline_messageslist = new ew_Form("foffline_messageslist", "list");
foffline_messageslist.FormKeyCountName = '<?php echo $offline_messages_list->FormKeyCountName ?>';

// Form_CustomValidate event
foffline_messageslist.Form_CustomValidate = 
...
foffline_messageslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php } ?>
<?php
	$bSelectLimit = $offline_messages_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($offline_messages_list->TotalRecs <= 0)
			$offline_messages_list->TotalRecs = $offline_messages->ListRecordCount();
	} else {
		if (!$offline_messages_list->Recordset && ($offline_messages_list->Recordset = $offline_messages_list->LoadRecordset()))
			$offline_messages_list->TotalRecs = $offline_messages_list->Recordset->RecordCount();
	}
	$offline_messages_list->StartRec = 1;
	if ($offline_messages_list->DisplayRecs <= 0 || ($offline_messages->Export <> "" && $offline_messages->ExportAll)) // Display all records
		$offline_messages_list->DisplayRecs = $offline_messages_list->TotalRecs;
	if (!($offline_messages->Export <> "" && $offline_messages->ExportAll))
		$offline_messages_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$offline_messages_list->Recordset = $offline_messages_list->LoadRecordset($offline_messages_list->StartRec-1, $offline_messages_list->DisplayRecs);

	// Set no record found message
	if ($offline_messages->CurrentAction == "" && $offline_messages_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$offline_messages_list->setWarningMessage(ew_DeniedMsg());
		if ($offline_messages_list->SearchWhere == "0=101")
			$offline_messages_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$offline_messages_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
...
?>
<?php if ($offline_messages_list->TotalRecs > 0 || $offline_messages->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($offline_messages_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> offline_messages">
<form name="foffline_messageslist" id="foffline_messageslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($offline_messages_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $offline_messages_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="offline_messages">
<div id="gmp_offline_messages" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($offline_messages_list->TotalRecs > 0 || $offline_messages->CurrentAction == "gridedit") { ?>
<table id="tbl_offline_messageslist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$offline_messages_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$offline_messages_list->RenderListOptions();

// Render list options (header, left)
$offline_messages_list->ListOptions->Render("header", "left");
?>
<?php if ($offline_messages->id->Visible) { // id ?>
	<?php if ($offline_messages->SortUrl($offline_messages->id) == "") { ?>
		<th data-name="id" class="<?php echo $offline_messages->id->HeaderCellClass() ?>"><div id="elh_offline_messages_id" class="offline_messages_id"><div class="ewTableHeaderCaption"><?php echo $offline_messages->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id" class="<?php echo $offline_messages->id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $offline_messages->SortUrl($offline_messages->id) ?>',1);"><div id="elh_offline_messages_id" class="offline_messages_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $offline_messages->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($offline_messages->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($offline_messages->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($offline_messages->timestamp->Visible) { // timestamp ?>
	<?php if ($offline_messages->SortUrl($offline_messages->timestamp) == "") { ?>
		<th data-name="timestamp" class="<?php echo $offline_messages->timestamp->HeaderCellClass() ?>"><div id="elh_offline_messages_timestamp" class="offline_messages_timestamp"><div class="ewTableHeaderCaption"><?php echo $offline_messages->timestamp->FldCaption() ?></div></div></th>
	<?php } else { ?>
...
$offline_messages_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($offline_messages->ExportAll && $offline_messages->Export <> "") {
	$offline_messages_list->StopRec = $offline_messages_list->TotalRecs;
} else {

	// Set the last record to display
	if ($offline_messages_list->TotalRecs > $offline_messages_list->StartRec + $offline_messages_list->DisplayRecs - 1)
		$offline_messages_list->StopRec = $offline_messages_list->StartRec + $offline_messages_list->DisplayRecs - 1;
	else
		$offline_messages_list->StopRec = $offline_messages_list->TotalRecs;
}
$offline_messages_list->RecCnt = $offline_messages_list->StartRec - 1;
if ($offline_messages_list->Recordset && !$offline_messages_list->Recordset->EOF) {
	$offline_messages_list->Recordset->MoveFirst();
	$bSelectLimit = $offline_messages_list->UseSelectLimit;
	if (!$bSelectLimit && $offline_messages_list->StartRec > 1)
		$offline_messages_list->Recordset->Move($offline_messages_list->StartRec - 1);
} elseif (!$offline_messages->AllowAddDeleteRow && $offline_messages_list->StopRec == 0) {
	$offline_messages_list->StopRec = $offline_messages->GridAddRowCount;
}

// Initialize aggregate
$offline_messages->RowType = EW_ROWTYPE_AGGREGATEINIT;
$offline_messages->ResetAttrs();
$offline_messages_list->RenderRow();
while ($offline_messages_list->RecCnt < $offline_messages_list->StopRec) {
	$offline_messages_list->RecCnt++;
	if (intval($offline_messages_list->RecCnt) >= intval($offline_messages_list->StartRec)) {
		$offline_messages_list->RowCnt++;

		// Set up key count
		$offline_messages_list->KeyCount = $offline_messages_list->RowIndex;

		// Init row class and style
		$offline_messages->ResetAttrs();
		$offline_messages->CssClass = "";
		if ($offline_messages->CurrentAction == "gridadd") {
		} else {
			$offline_messages_list->LoadRowValues($offline_messages_list->Recordset); // Load row values
		}
		$offline_messages->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$offline_messages->RowAttrs = array_merge($offline_messages->RowAttrs, array('data-rowindex'=>$offline_messages_list->RowCnt, 'id'=>'r' . $offline_messages_list->RowCnt . '_offline_messages', 'data-rowtype'=>$offline_messages->RowType));
		...
		$offline_messages_list->RenderListOptions();
?>
	<tr<?php echo $offline_messages->RowAttributes() ?>>
<?php

// Render list options (body, left)
$offline_messages_list->ListOptions->Render("body", "left", $offline_messages_list->RowCnt);
?>
	<?php if ($offline_messages->id->Visible) { // id ?>
		<td data-name="id"<?php echo $offline_messages->id->CellAttributes() ?>>
<span id="el<?php echo $offline_messages_list->RowCnt ?>_offline_messages_id" class="offline_messages_id">
<span<?php echo $offline_messages->id->ViewAttributes() ?>>
<?php echo $offline_messages->id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($offline_messages->timestamp->Visible) { // timestamp ?>
		<td data-name="timestamp"<?php echo $offline_messages->timestamp->CellAttributes() ?>>
...
$offline_messages_list->ListOptions->Render("body", "right", $offline_messages_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($offline_messages->CurrentAction <> "gridadd")
		$offline_messages_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($offline_messages->CurrentAction == "") { ?>
...
// Close recordset
if ($offline_messages_list->Recordset)
	$offline_messages_list->Recordset->Close();
?>
<?php if ($offline_messages->Export == "") { ?>
<div class="box-footer ewGridLowerPanel">
<?php if ($offline_messages->CurrentAction <> "gridadd" && $offline_messages->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($offline_messages_list->Pager)) $offline_messages_list->Pager = new cNumericPager($offline_messages_list->StartRec, $offline_messages_list->DisplayRecs, $offline_messages_list->TotalRecs, $offline_messages_list->RecRange, $offline_messages_list->AutoHidePager) ?>
<?php if ($offline_messages_list->Pager->RecordCount > 0 && $offline_messages_list->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($offline_messages_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $offline_messages_list->PageUrl() ?>start=<?php echo $offline_messages_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($offline_messages_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $offline_messages_list->PageUrl() ?>start=<?php echo $offline_messages_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($offline_messages_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $offline_messages_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($offline_messages_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $offline_messages_list->PageUrl() ?>start=<?php echo $offline_messages_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($offline_messages_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $offline_messages_list->PageUrl() ?>start=<?php echo $offline_messages_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
<?php if ($offline_messages_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $offline_messages_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $offline_messages_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $offline_messages_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($offline_messages_list->TotalRecs > 0 && (!$offline_messages_list->AutoHidePageSizeSelector || $offline_messages_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="offline_messages">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($offline_messages_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($offline_messages_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($offline_messages_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($offline_messages->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($offline_messages_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($offline_messages_list->TotalRecs == 0 && $offline_messages->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($offline_messages_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<?php
$offline_messages_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($offline_messages->Export == "") { ?>
...
<?php
$offline_messages_list->Page_Terminate();
?>
