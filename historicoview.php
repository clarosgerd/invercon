...
class chistorico_list extends chistorico {
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fhistoricolistsrch");
		$sFilterList = ew_Concat($sFilterList, $this->id->AdvancedSearch->ToJson(), ","); // Field id
		$sFilterList = ew_Concat($sFilterList, $this->proceso->AdvancedSearch->ToJson(), ","); // Field proceso
		$sFilterList = ew_Concat($sFilterList, $this->responsable->AdvancedSearch->ToJson(), ","); // Field responsable
		$sFilterList = ew_Concat($sFilterList, $this->ingreso->AdvancedSearch->ToJson(), ","); // Field ingreso
		$sFilterList = ew_Concat($sFilterList, $this->salida->AdvancedSearch->ToJson(), ","); // Field salida
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fhistoricolistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field responsable
		$this->responsable->AdvancedSearch->SearchValue = @$filter["x_responsable"];
		$this->responsable->AdvancedSearch->SearchOperator = @$filter["z_responsable"];
		$this->responsable->AdvancedSearch->SearchCondition = @$filter["v_responsable"];
		$this->responsable->AdvancedSearch->SearchValue2 = @$filter["y_responsable"];
		$this->responsable->AdvancedSearch->SearchOperator2 = @$filter["w_responsable"];
		$this->responsable->AdvancedSearch->Save();

		// Field ingreso
		$this->ingreso->AdvancedSearch->SearchValue = @$filter["x_ingreso"];
		$this->ingreso->AdvancedSearch->SearchOperator = @$filter["z_ingreso"];
		$this->ingreso->AdvancedSearch->SearchCondition = @$filter["v_ingreso"];
		$this->ingreso->AdvancedSearch->SearchValue2 = @$filter["y_ingreso"];
		$this->ingreso->AdvancedSearch->SearchOperator2 = @$filter["w_ingreso"];
		$this->ingreso->AdvancedSearch->Save();
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fhistoricolistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_historico\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_historico',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fhistoricolist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
fhistoricolist.Lists["x_responsable"] = {"LinkField":"x__login","Ajax":true,"AutoFill":false,"DisplayFields":["x_codigo","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"usuario"};
fhistoricolist.Lists["x_responsable"].Data = "<?php echo $historico_list->responsable->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fhistoricolistsrch = new ew_Form("fhistoricolistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($historico_list->SearchOptions->Visible()) { ?>
<?php $historico_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($historico_list->FilterOptions->Visible()) { ?>
<?php $historico_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php if ($historico->Export == "" && $historico->CurrentAction == "") { ?>
<form name="fhistoricolistsrch" id="fhistoricolistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($historico_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fhistoricolistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="historico">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($historico_list->BasicSearch->getKeyword()) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($historico_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $historico_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($historico_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($historico_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($historico_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($historico_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php if ($historico->proceso->Visible) { // proceso ?>
	...
	<?php } else { ?>
		<th data-name="proceso" class="<?php echo $historico->proceso->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $historico->SortUrl($historico->proceso) ?>',1);"><div id="elh_historico_proceso" class="historico_proceso">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $historico->proceso->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($historico->proceso->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($historico->proceso->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($historico->Export == "") { ?>
<script type="text/javascript">
fhistoricolistsrch.FilterList = <?php echo $historico_list->GetFilterList() ?>;
fhistoricolistsrch.Init();
fhistoricolist.Init();
</script>
<?php } ?>
<?php
