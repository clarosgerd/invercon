...
$solicitud_add = NULL; // Initialize page object first

class csolicitud_add extends csolicitud {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "solicitudview.php")
	...
	function LoadDefaultValues() {
		...
		$this->email_contacto->CurrentValue = NULL;
		$this->email_contacto->OldValue = $this->email_contacto->CurrentValue;
		...
		$this->monto_inicial->CurrentValue = NULL;
		$this->monto_inicial->OldValue = $this->monto_inicial->CurrentValue;
	}
	...
	function NewRow() {
		...
		$row['monto_inicial'] = $this->monto_inicial->CurrentValue;
		return $row;
	}
	...
	function RenderRow() {
		...
		$this->monto_inicial->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->imagen_vehiculo02);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->imagen_maquinaria02);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->imagen_mercaderia01);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->imagen_tipoespecial01);
	...
	function ValidateForm() {
		...
		if (in_array("avaluo", $DetailTblVar) && $GLOBALS["avaluo"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// imagen_vehiculo02
		if ($this->imagen_vehiculo02->Visible && !$this->imagen_vehiculo02->Upload->KeepFile) {
		...
		// imagen_maquinaria02
		if ($this->imagen_maquinaria02->Visible && !$this->imagen_maquinaria02->Upload->KeepFile) {
		...
		// imagen_mercaderia01
		if ($this->imagen_mercaderia01->Visible && !$this->imagen_mercaderia01->Upload->KeepFile) {
		...
		// imagen_tipoespecial01
		if ($this->imagen_tipoespecial01->Visible && !$this->imagen_tipoespecial01->Upload->KeepFile) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("avaluo", $DetailTblVar) && $GLOBALS["avaluo"]->DetailAdd) {
				$GLOBALS["avaluo"]->id_solicitud->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["avaluo_grid"])) $GLOBALS["avaluo_grid"] = new cavaluo_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "avaluo"); // Load user level of detail table
				$AddRow = $GLOBALS["avaluo_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["avaluo"]->id_solicitud->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("avaluo", $DetailTblVar)) {
				...
				if ($GLOBALS["avaluo_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["avaluo_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["avaluo_grid"]->CurrentMode = "add";
					$GLOBALS["avaluo_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($solicitud_add)) $solicitud_add = new csolicitud_add();

// Page init
$solicitud_add->Page_Init();

// Page main
$solicitud_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$solicitud_add->Page_Render();
?>
...
fsolicitudadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
fsolicitudadd.MultiPage = new ew_MultiPage("fsolicitudadd");

// Dynamic selection lists
fsolicitudadd.Lists["x_tipoinmueble[]"] = {"LinkField":"x_nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fsolicitudadd.Lists["x_tipoinmueble[]"].Data = "<?php echo $solicitud_add->tipoinmueble->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_id_ciudad_inmueble"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departamento"};
fsolicitudadd.Lists["x_id_ciudad_inmueble"].Data = "<?php echo $solicitud_add->id_ciudad_inmueble->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_id_provincia_inmueble"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"provincia"};
fsolicitudadd.Lists["x_id_provincia_inmueble"].Data = "<?php echo $solicitud_add->id_provincia_inmueble->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_tipovehiculo[]"] = {"LinkField":"x_nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fsolicitudadd.Lists["x_tipovehiculo[]"].Data = "<?php echo $solicitud_add->tipovehiculo->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_id_ciudad_vehiculo"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departamento"};
fsolicitudadd.Lists["x_id_ciudad_vehiculo"].Data = "<?php echo $solicitud_add->id_ciudad_vehiculo->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_id_provincia_vehiculo"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"provincia"};
fsolicitudadd.Lists["x_id_provincia_vehiculo"].Data = "<?php echo $solicitud_add->id_provincia_vehiculo->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_tipomaquinaria[]"] = {"LinkField":"x_nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fsolicitudadd.Lists["x_tipomaquinaria[]"].Data = "<?php echo $solicitud_add->tipomaquinaria->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_id_ciudad_maquinaria"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"departamento"};
fsolicitudadd.Lists["x_id_ciudad_maquinaria"].Data = "<?php echo $solicitud_add->id_ciudad_maquinaria->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_id_provincia_maquinaria"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"provincia"};
fsolicitudadd.Lists["x_id_provincia_maquinaria"].Data = "<?php echo $solicitud_add->id_provincia_maquinaria->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_tipomercaderia[]"] = {"LinkField":"x_id_tipoinmueble","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fsolicitudadd.Lists["x_tipomercaderia[]"].Data = "<?php echo $solicitud_add->tipomercaderia->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_tipoespecial[]"] = {"LinkField":"x_id_tipoinmueble","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fsolicitudadd.Lists["x_tipoespecial[]"].Data = "<?php echo $solicitud_add->tipoespecial->LookupFilterQuery(FALSE, "add") ?>";
fsolicitudadd.Lists["x_email_contacto"] = {"LinkField":"x__login","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellido","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"oficialcredito"};
fsolicitudadd.Lists["x_email_contacto"].Data = "<?php echo $solicitud_add->email_contacto->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $solicitud_add->ShowPageHeader(); ?>
<?php
$solicitud_add->ShowMessage();
?>
<form name="fsolicitudadd" id="fsolicitudadd" class="<?php echo $solicitud_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($solicitud_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $solicitud_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="solicitud">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($solicitud_add->IsModal) ?>">
<?php if (!$solicitud_add->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="solicitud_add"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $solicitud_add->MultiPages->NavStyle() ?>">
		<li<?php echo $solicitud_add->MultiPages->TabStyle("1") ?>><a href="#tab_solicitud1" data-toggle="tab"><?php echo $solicitud->PageCaption(1) ?></a></li>
		<li<?php echo $solicitud_add->MultiPages->TabStyle("2") ?>><a href="#tab_solicitud2" data-toggle="tab"><?php echo $solicitud->PageCaption(2) ?></a></li>
		<li<?php echo $solicitud_add->MultiPages->TabStyle("3") ?>><a href="#tab_solicitud3" data-toggle="tab"><?php echo $solicitud->PageCaption(3) ?></a></li>
		<li<?php echo $solicitud_add->MultiPages->TabStyle("4") ?>><a href="#tab_solicitud4" data-toggle="tab"><?php echo $solicitud->PageCaption(4) ?></a></li>
		<li<?php echo $solicitud_add->MultiPages->TabStyle("5") ?>><a href="#tab_solicitud5" data-toggle="tab"><?php echo $solicitud->PageCaption(5) ?></a></li>
		<li<?php echo $solicitud_add->MultiPages->TabStyle("6") ?>><a href="#tab_solicitud6" data-toggle="tab"><?php echo $solicitud->PageCaption(6) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $solicitud_add->MultiPages->PageStyle("1") ?>" id="tab_solicitud1"><!-- multi-page .tab-pane -->
<?php if ($solicitud_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_solicitudadd1" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($solicitud->name->Visible) { // name ?>
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_name" class="form-group">
		<label id="elh_solicitud_name" for="x_name" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->name->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r__email" class="form-group">
		<label id="elh_solicitud__email" for="x__email" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->_email->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->_email->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_address" class="form-group">
		<label id="elh_solicitud_address" for="x_address" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->address->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->address->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_phone" class="form-group">
		<label id="elh_solicitud_phone" for="x_phone" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->phone->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->phone->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_cell" class="form-group">
		<label id="elh_solicitud_cell" for="x_cell" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->cell->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->cell->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_email_contacto" class="form-group">
		<label id="elh_solicitud_email_contacto" for="x_email_contacto" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->email_contacto->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->email_contacto->CellAttributes() ?>>
<span id="el_solicitud_email_contacto">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_email_contacto"><?php echo (strval($solicitud->email_contacto->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $solicitud->email_contacto->ViewValue); ?></span>
</span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_nombre_contacto" class="form-group">
		<label id="elh_solicitud_nombre_contacto" for="x_nombre_contacto" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->nombre_contacto->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->nombre_contacto->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_lastname" class="form-group">
		<label id="elh_solicitud_lastname" for="x_lastname" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->lastname->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->lastname->CellAttributes() ?>>
...
<?php } else { ?>
	<tr id="r_lastname">
		<td class="col-sm-3"><span id="elh_solicitud_lastname"><?php echo $solicitud->lastname->FldCaption() ?></span></td>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $solicitud_add->MultiPages->PageStyle("2") ?>" id="tab_solicitud2"><!-- multi-page .tab-pane -->
<?php if ($solicitud_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_solicitudadd2" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($solicitud->tipoinmueble->Visible) { // tipoinmueble ?>
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_tipoinmueble" class="form-group">
		<label id="elh_solicitud_tipoinmueble" for="x_tipoinmueble" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->tipoinmueble->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->tipoinmueble->CellAttributes() ?>>
<span id="el_solicitud_tipoinmueble">
<div class="ewDropdownList has-feedback">
	<span onclick="" class="form-control dropdown-toggle" aria-expanded="false"<?php if ($solicitud->tipoinmueble->ReadOnly) { ?> readonly<?php } else { ?>data-toggle="dropdown"<?php } ?>>
		<?php echo $solicitud->tipoinmueble->ViewValue ?>
	</span>
	<?php if (!$solicitud->tipoinmueble->ReadOnly) { ?>
	<span class="glyphicon glyphicon-remove form-control-feedback ewDropdownListClear"></span>
	<span class="form-control-feedback"><span class="caret"></span></span>
	<?php } ?>
	<div id="dsl_x_tipoinmueble" data-repeatcolumn="1" class="dropdown-menu">
		<div class="ewItems" style="position: relative; overflow-x: hidden;">
<?php echo $solicitud->tipoinmueble->CheckBoxListHtml(TRUE, "x_tipoinmueble[]", 2) ?>
		</div>
	</div>
	<div id="tp_x_tipoinmueble" class="ewTemplate"><input type="checkbox" data-table="solicitud" data-field="x_tipoinmueble" data-page="2" data-value-separator="<?php echo $solicitud->tipoinmueble->DisplayValueSeparatorAttribute() ?>" name="x_tipoinmueble[]" id="x_tipoinmueble[]" value="{value}"<?php echo $solicitud->tipoinmueble->EditAttributes() ?>></div>
</div>
</span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_id_ciudad_inmueble" class="form-group">
		<label id="elh_solicitud_id_ciudad_inmueble" for="x_id_ciudad_inmueble" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->id_ciudad_inmueble->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->id_ciudad_inmueble->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_id_provincia_inmueble" class="form-group">
		<label id="elh_solicitud_id_provincia_inmueble" for="x_id_provincia_inmueble" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->id_provincia_inmueble->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->id_provincia_inmueble->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_inmueble02" class="form-group">
		<label id="elh_solicitud_imagen_inmueble02" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_inmueble02->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_inmueble02->CellAttributes() ?>>
<span id="el_solicitud_imagen_inmueble02">
<div id="fd_x_imagen_inmueble02">
<span title="<?php echo $solicitud->imagen_inmueble02->FldTitle() ? $solicitud->imagen_inmueble02->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_inmueble02->ReadOnly || $solicitud->imagen_inmueble02->Disabled) echo " hide"; ?>">
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_inmueble03" class="form-group">
		<label id="elh_solicitud_imagen_inmueble03" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_inmueble03->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_inmueble03->CellAttributes() ?>>
<span id="el_solicitud_imagen_inmueble03">
<div id="fd_x_imagen_inmueble03">
<span title="<?php echo $solicitud->imagen_inmueble03->FldTitle() ? $solicitud->imagen_inmueble03->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_inmueble03->ReadOnly || $solicitud->imagen_inmueble03->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_inmueble04" class="form-group">
		<label id="elh_solicitud_imagen_inmueble04" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_inmueble04->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_inmueble04->CellAttributes() ?>>
<span id="el_solicitud_imagen_inmueble04">
<div id="fd_x_imagen_inmueble04">
<span title="<?php echo $solicitud->imagen_inmueble04->FldTitle() ? $solicitud->imagen_inmueble04->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_inmueble04->ReadOnly || $solicitud->imagen_inmueble04->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_inmueble05" class="form-group">
		<label id="elh_solicitud_imagen_inmueble05" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_inmueble05->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_inmueble05->CellAttributes() ?>>
<span id="el_solicitud_imagen_inmueble05">
<div id="fd_x_imagen_inmueble05">
<span title="<?php echo $solicitud->imagen_inmueble05->FldTitle() ? $solicitud->imagen_inmueble05->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_inmueble05->ReadOnly || $solicitud->imagen_inmueble05->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $solicitud_add->MultiPages->PageStyle("3") ?>" id="tab_solicitud3"><!-- multi-page .tab-pane -->
<?php if ($solicitud_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_solicitudadd3" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($solicitud->tipovehiculo->Visible) { // tipovehiculo ?>
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_tipovehiculo" class="form-group">
		<label id="elh_solicitud_tipovehiculo" for="x_tipovehiculo" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->tipovehiculo->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->tipovehiculo->CellAttributes() ?>>
<span id="el_solicitud_tipovehiculo">
<select data-table="solicitud" data-field="x_tipovehiculo" data-page="3" data-value-separator="<?php echo $solicitud->tipovehiculo->DisplayValueSeparatorAttribute() ?>" id="x_tipovehiculo[]" name="x_tipovehiculo[]" multiple="multiple"<?php echo $solicitud->tipovehiculo->EditAttributes() ?>>
<?php echo $solicitud->tipovehiculo->SelectOptionListHtml("x_tipovehiculo[]") ?>
</select>
</span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_id_ciudad_vehiculo" class="form-group">
		<label id="elh_solicitud_id_ciudad_vehiculo" for="x_id_ciudad_vehiculo" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->id_ciudad_vehiculo->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->id_ciudad_vehiculo->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_id_provincia_vehiculo" class="form-group">
		<label id="elh_solicitud_id_provincia_vehiculo" for="x_id_provincia_vehiculo" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->id_provincia_vehiculo->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->id_provincia_vehiculo->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_vehiculo02" class="form-group">
		<label id="elh_solicitud_imagen_vehiculo02" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_vehiculo02->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_vehiculo02->CellAttributes() ?>>
<span id="el_solicitud_imagen_vehiculo02">
<div id="fd_x_imagen_vehiculo02">
<span title="<?php echo $solicitud->imagen_vehiculo02->FldTitle() ? $solicitud->imagen_vehiculo02->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_vehiculo02->ReadOnly || $solicitud->imagen_vehiculo02->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_vehiculo05" class="form-group">
		<label id="elh_solicitud_imagen_vehiculo05" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_vehiculo05->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_vehiculo05->CellAttributes() ?>>
<span id="el_solicitud_imagen_vehiculo05">
<div id="fd_x_imagen_vehiculo05">
<span title="<?php echo $solicitud->imagen_vehiculo05->FldTitle() ? $solicitud->imagen_vehiculo05->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_vehiculo05->ReadOnly || $solicitud->imagen_vehiculo05->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_vehiculo06" class="form-group">
		<label id="elh_solicitud_imagen_vehiculo06" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_vehiculo06->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_vehiculo06->CellAttributes() ?>>
<span id="el_solicitud_imagen_vehiculo06">
<div id="fd_x_imagen_vehiculo06">
<span title="<?php echo $solicitud->imagen_vehiculo06->FldTitle() ? $solicitud->imagen_vehiculo06->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_vehiculo06->ReadOnly || $solicitud->imagen_vehiculo06->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_vehiculo07" class="form-group">
		<label id="elh_solicitud_imagen_vehiculo07" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_vehiculo07->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_vehiculo07->CellAttributes() ?>>
<span id="el_solicitud_imagen_vehiculo07">
<div id="fd_x_imagen_vehiculo07">
<span title="<?php echo $solicitud->imagen_vehiculo07->FldTitle() ? $solicitud->imagen_vehiculo07->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_vehiculo07->ReadOnly || $solicitud->imagen_vehiculo07->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $solicitud_add->MultiPages->PageStyle("4") ?>" id="tab_solicitud4"><!-- multi-page .tab-pane -->
<?php if ($solicitud_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_solicitudadd4" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($solicitud->tipomaquinaria->Visible) { // tipomaquinaria ?>
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_tipomaquinaria" class="form-group">
		<label id="elh_solicitud_tipomaquinaria" for="x_tipomaquinaria" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->tipomaquinaria->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->tipomaquinaria->CellAttributes() ?>>
<span id="el_solicitud_tipomaquinaria">
<select data-table="solicitud" data-field="x_tipomaquinaria" data-page="4" data-value-separator="<?php echo $solicitud->tipomaquinaria->DisplayValueSeparatorAttribute() ?>" id="x_tipomaquinaria[]" name="x_tipomaquinaria[]" multiple="multiple"<?php echo $solicitud->tipomaquinaria->EditAttributes() ?>>
<?php echo $solicitud->tipomaquinaria->SelectOptionListHtml("x_tipomaquinaria[]") ?>
</select>
</span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_id_ciudad_maquinaria" class="form-group">
		<label id="elh_solicitud_id_ciudad_maquinaria" for="x_id_ciudad_maquinaria" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->id_ciudad_maquinaria->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->id_ciudad_maquinaria->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_id_provincia_maquinaria" class="form-group">
		<label id="elh_solicitud_id_provincia_maquinaria" for="x_id_provincia_maquinaria" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->id_provincia_maquinaria->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->id_provincia_maquinaria->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_maquinaria02" class="form-group">
		<label id="elh_solicitud_imagen_maquinaria02" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_maquinaria02->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_maquinaria02->CellAttributes() ?>>
<span id="el_solicitud_imagen_maquinaria02">
<div id="fd_x_imagen_maquinaria02">
<span title="<?php echo $solicitud->imagen_maquinaria02->FldTitle() ? $solicitud->imagen_maquinaria02->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_maquinaria02->ReadOnly || $solicitud->imagen_maquinaria02->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_maquinaria05" class="form-group">
		<label id="elh_solicitud_imagen_maquinaria05" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_maquinaria05->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_maquinaria05->CellAttributes() ?>>
<span id="el_solicitud_imagen_maquinaria05">
<div id="fd_x_imagen_maquinaria05">
<span title="<?php echo $solicitud->imagen_maquinaria05->FldTitle() ? $solicitud->imagen_maquinaria05->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_maquinaria05->ReadOnly || $solicitud->imagen_maquinaria05->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_maquinaria06" class="form-group">
		<label id="elh_solicitud_imagen_maquinaria06" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_maquinaria06->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_maquinaria06->CellAttributes() ?>>
<span id="el_solicitud_imagen_maquinaria06">
<div id="fd_x_imagen_maquinaria06">
<span title="<?php echo $solicitud->imagen_maquinaria06->FldTitle() ? $solicitud->imagen_maquinaria06->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_maquinaria06->ReadOnly || $solicitud->imagen_maquinaria06->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_maquinaria07" class="form-group">
		<label id="elh_solicitud_imagen_maquinaria07" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_maquinaria07->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_maquinaria07->CellAttributes() ?>>
<span id="el_solicitud_imagen_maquinaria07">
<div id="fd_x_imagen_maquinaria07">
<span title="<?php echo $solicitud->imagen_maquinaria07->FldTitle() ? $solicitud->imagen_maquinaria07->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_maquinaria07->ReadOnly || $solicitud->imagen_maquinaria07->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $solicitud_add->MultiPages->PageStyle("5") ?>" id="tab_solicitud5"><!-- multi-page .tab-pane -->
<?php if ($solicitud_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_solicitudadd5" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($solicitud->tipomercaderia->Visible) { // tipomercaderia ?>
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_tipomercaderia" class="form-group">
		<label id="elh_solicitud_tipomercaderia" for="x_tipomercaderia" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->tipomercaderia->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->tipomercaderia->CellAttributes() ?>>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_mercaderia01" class="form-group">
		<label id="elh_solicitud_imagen_mercaderia01" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_mercaderia01->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_mercaderia01->CellAttributes() ?>>
<span id="el_solicitud_imagen_mercaderia01">
<div id="fd_x_imagen_mercaderia01">
<span title="<?php echo $solicitud->imagen_mercaderia01->FldTitle() ? $solicitud->imagen_mercaderia01->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_mercaderia01->ReadOnly || $solicitud->imagen_mercaderia01->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_documento_mercaderia" class="form-group">
		<label id="elh_solicitud_documento_mercaderia" for="x_documento_mercaderia" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->documento_mercaderia->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->documento_mercaderia->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $solicitud_add->MultiPages->PageStyle("6") ?>" id="tab_solicitud6"><!-- multi-page .tab-pane -->
<?php if ($solicitud_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_solicitudadd6" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($solicitud->tipoespecial->Visible) { // tipoespecial ?>
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_tipoespecial" class="form-group">
		<label id="elh_solicitud_tipoespecial" for="x_tipoespecial" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->tipoespecial->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->tipoespecial->CellAttributes() ?>>
<span id="el_solicitud_tipoespecial">
<select data-table="solicitud" data-field="x_tipoespecial" data-page="6" data-value-separator="<?php echo $solicitud->tipoespecial->DisplayValueSeparatorAttribute() ?>" id="x_tipoespecial[]" name="x_tipoespecial[]" multiple="multiple"<?php echo $solicitud->tipoespecial->EditAttributes() ?>>
<?php echo $solicitud->tipoespecial->SelectOptionListHtml("x_tipoespecial[]") ?>
</select>
</span>
...
<?php if ($solicitud_add->IsMobileOrModal) { ?>
	<div id="r_imagen_tipoespecial01" class="form-group">
		<label id="elh_solicitud_imagen_tipoespecial01" class="<?php echo $solicitud_add->LeftColumnClass ?>"><?php echo $solicitud->imagen_tipoespecial01->FldCaption() ?></label>
		<div class="<?php echo $solicitud_add->RightColumnClass ?>"><div<?php echo $solicitud->imagen_tipoespecial01->CellAttributes() ?>>
<span id="el_solicitud_imagen_tipoespecial01">
<div id="fd_x_imagen_tipoespecial01">
<span title="<?php echo $solicitud->imagen_tipoespecial01->FldTitle() ? $solicitud->imagen_tipoespecial01->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($solicitud->imagen_tipoespecial01->ReadOnly || $solicitud->imagen_tipoespecial01->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
...
<?php
	if (in_array("avaluo", explode(",", $solicitud->getCurrentDetailTable())) && $avaluo->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$solicitud_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $solicitud_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $solicitud_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
