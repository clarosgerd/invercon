...
function CurrentProjectID() {
	...
	return "{30AA0C25-B486-48CC-AF92-47D039BF725C}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionParentUserID($rs->fields('cargo')); // Load parent User ID
	...
	function CurrentUserEmail() {
		return $this->CurrentUserInfo("login");
	}
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`cargo` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('login'));
