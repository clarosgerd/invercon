...
class ccliente_view extends ccliente {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("clientelist.php"); // Return to list page
	...
	function SetupOtherOptions() {
		...
		// "detail_pago"
		$item = &$option->Add("detail_pago");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("pago", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["pago_grid"] && $GLOBALS["pago_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'pago')) {
			$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=pago")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
			if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
			$DetailViewTblVar .= "pago";
		}
		if ($GLOBALS["pago_grid"] && $GLOBALS["pago_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'pago')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=pago")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'pago');
		if ($item->Visible) {
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("pago", $DetailTblVar)) {
				...
				if ($GLOBALS["pago_grid"]->DetailView) {
					$GLOBALS["pago_grid"]->CurrentMode = "view";
...
fclienteview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<input type="hidden" name="modal" value="<?php echo intval($cliente_view->IsModal) ?>">
...
<?php echo $cliente->name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $cliente->lastname->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $cliente->address->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $cliente->phone->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $cliente->phone_cell->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $cliente->is_active->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
</table>
<?php if (!$cliente_view->IsModal) { ?>
<?php if (!isset($cliente_view->Pager)) $cliente_view->Pager = new cNumericPager($cliente_view->StartRec, $cliente_view->DisplayRecs, $cliente_view->TotalRecs, $cliente_view->RecRange, $cliente_view->AutoHidePager) ?>
<?php if ($cliente_view->Pager->RecordCount > 0 && $cliente_view->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($cliente_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_view->PageUrl() ?>start=<?php echo $cliente_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($cliente_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_view->PageUrl() ?>start=<?php echo $cliente_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($cliente_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $cliente_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($cliente_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_view->PageUrl() ?>start=<?php echo $cliente_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($cliente_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_view->PageUrl() ?>start=<?php echo $cliente_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("pago", explode(",", $cliente->getCurrentDetailTable())) && $pago->DetailView) {
?>
