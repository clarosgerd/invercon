...
$viewsolicitudinspector_list = NULL; // Initialize page object first

class cviewsolicitudinspector_list extends cviewsolicitudinspector {
	...
	// Page object name
	var $PageObjName = 'viewsolicitudinspector_list';

	// Grid form hidden field names
	var $FormName = 'fviewsolicitudinspectorlist';
	...
	function __construct() {
		...
		$this->AddUrl = "viewsolicitudinspectoradd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "viewsolicitudinspectordelete.php";
		$this->MultiUpdateUrl = "viewsolicitudinspectorupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fviewsolicitudinspectorlistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fviewsolicitudinspectorlistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fviewsolicitudinspectorlistsrch", $filters);
	...
	function SetupListOptions() {
		...
		// "detail_viewavaluoinspector"
		$item = &$this->ListOptions->Add("detail_viewavaluoinspector");
		$item->CssClass = "text-nowrap";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'viewavaluoinspector') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_viewavaluoinspector"
		$oListOpt = &$this->ListOptions->Items["detail_viewavaluoinspector"];
		if ($Security->AllowList(CurrentProjectID() . 'viewavaluoinspector')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("viewavaluoinspector", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("viewavaluoinspectorlist.php?" . EW_TABLE_SHOW_MASTER . "=viewsolicitudinspector&fk_id=" . urlencode(strval($this->id->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fviewsolicitudinspectorlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fviewsolicitudinspectorlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fviewsolicitudinspectorlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fviewsolicitudinspectorlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_viewsolicitudinspector\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_viewsolicitudinspector',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fviewsolicitudinspectorlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($viewsolicitudinspector_list)) $viewsolicitudinspector_list = new cviewsolicitudinspector_list();

// Page init
$viewsolicitudinspector_list->Page_Init();

// Page main
$viewsolicitudinspector_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$viewsolicitudinspector_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($viewsolicitudinspector->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fviewsolicitudinspectorlist = new ew_Form("fviewsolicitudinspectorlist", "list");
fviewsolicitudinspectorlist.FormKeyCountName = '<?php echo $viewsolicitudinspector_list->FormKeyCountName ?>';

// Form_CustomValidate event
fviewsolicitudinspectorlist.Form_CustomValidate = 
...
fviewsolicitudinspectorlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fviewsolicitudinspectorlist.Lists["x_tipoinmueble[]"] = {"LinkField":"x_nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fviewsolicitudinspectorlist.Lists["x_tipoinmueble[]"].Data = "<?php echo $viewsolicitudinspector_list->tipoinmueble->LookupFilterQuery(FALSE, "list") ?>";
fviewsolicitudinspectorlist.Lists["x_tipovehiculo[]"] = {"LinkField":"x_nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fviewsolicitudinspectorlist.Lists["x_tipovehiculo[]"].Data = "<?php echo $viewsolicitudinspector_list->tipovehiculo->LookupFilterQuery(FALSE, "list") ?>";
fviewsolicitudinspectorlist.Lists["x_tipomaquinaria[]"] = {"LinkField":"x_nombre","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fviewsolicitudinspectorlist.Lists["x_tipomaquinaria[]"].Data = "<?php echo $viewsolicitudinspector_list->tipomaquinaria->LookupFilterQuery(FALSE, "list") ?>";
fviewsolicitudinspectorlist.Lists["x_tipomercaderia[]"] = {"LinkField":"x_id_tipoinmueble","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fviewsolicitudinspectorlist.Lists["x_tipomercaderia[]"].Data = "<?php echo $viewsolicitudinspector_list->tipomercaderia->LookupFilterQuery(FALSE, "list") ?>";
fviewsolicitudinspectorlist.Lists["x_tipoespecial"] = {"LinkField":"x_id_tipoinmueble","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipoinmueble"};
fviewsolicitudinspectorlist.Lists["x_tipoespecial"].Data = "<?php echo $viewsolicitudinspector_list->tipoespecial->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fviewsolicitudinspectorlistsrch = new ew_Form("fviewsolicitudinspectorlistsrch");

// Validate function for search
fviewsolicitudinspectorlistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
fviewsolicitudinspectorlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fviewsolicitudinspectorlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php if ($viewsolicitudinspector->Export == "") { ?>
<div class="ewToolbar">
<?php if ($viewsolicitudinspector_list->TotalRecs > 0 && $viewsolicitudinspector_list->ExportOptions->Visible()) { ?>
<?php $viewsolicitudinspector_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($viewsolicitudinspector_list->SearchOptions->Visible()) { ?>
<?php $viewsolicitudinspector_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($viewsolicitudinspector_list->FilterOptions->Visible()) { ?>
<?php $viewsolicitudinspector_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $viewsolicitudinspector_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($viewsolicitudinspector_list->TotalRecs <= 0)
			$viewsolicitudinspector_list->TotalRecs = $viewsolicitudinspector->ListRecordCount();
	} else {
		if (!$viewsolicitudinspector_list->Recordset && ($viewsolicitudinspector_list->Recordset = $viewsolicitudinspector_list->LoadRecordset()))
			$viewsolicitudinspector_list->TotalRecs = $viewsolicitudinspector_list->Recordset->RecordCount();
	}
	$viewsolicitudinspector_list->StartRec = 1;
	if ($viewsolicitudinspector_list->DisplayRecs <= 0 || ($viewsolicitudinspector->Export <> "" && $viewsolicitudinspector->ExportAll)) // Display all records
		$viewsolicitudinspector_list->DisplayRecs = $viewsolicitudinspector_list->TotalRecs;
	if (!($viewsolicitudinspector->Export <> "" && $viewsolicitudinspector->ExportAll))
		$viewsolicitudinspector_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$viewsolicitudinspector_list->Recordset = $viewsolicitudinspector_list->LoadRecordset($viewsolicitudinspector_list->StartRec-1, $viewsolicitudinspector_list->DisplayRecs);

	// Set no record found message
	if ($viewsolicitudinspector->CurrentAction == "" && $viewsolicitudinspector_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$viewsolicitudinspector_list->setWarningMessage(ew_DeniedMsg());
		if ($viewsolicitudinspector_list->SearchWhere == "0=101")
			$viewsolicitudinspector_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$viewsolicitudinspector_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$viewsolicitudinspector_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($viewsolicitudinspector->Export == "" && $viewsolicitudinspector->CurrentAction == "") { ?>
<form name="fviewsolicitudinspectorlistsrch" id="fviewsolicitudinspectorlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($viewsolicitudinspector_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fviewsolicitudinspectorlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="viewsolicitudinspector">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$viewsolicitudinspector_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$viewsolicitudinspector->RowType = EW_ROWTYPE_SEARCH;

// Render row
$viewsolicitudinspector->ResetAttrs();
$viewsolicitudinspector_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($viewsolicitudinspector->name->Visible) { // name ?>
	<div id="xsc_name" class="ewCell form-group">
		<label for="x_name" class="ewSearchCaption ewLabel"><?php echo $viewsolicitudinspector->name->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_name" id="z_name" value="LIKE"></span>
		<span class="ewSearchField">
<input type="text" data-table="viewsolicitudinspector" data-field="x_name" name="x_name" id="x_name" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($viewsolicitudinspector->name->getPlaceHolder()) ?>" value="<?php echo $viewsolicitudinspector->name->EditValue ?>"<?php echo $viewsolicitudinspector->name->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
...
<?php if ($viewsolicitudinspector->lastname->Visible) { // lastname ?>
	<div id="xsc_lastname" class="ewCell form-group">
		<label for="x_lastname" class="ewSearchCaption ewLabel"><?php echo $viewsolicitudinspector->lastname->FldCaption() ?></label>
...
<?php if ($viewsolicitudinspector->_email->Visible) { // email ?>
	<div id="xsc__email" class="ewCell form-group">
		<label for="x__email" class="ewSearchCaption ewLabel"><?php echo $viewsolicitudinspector->_email->FldCaption() ?></label>
...
<?php if ($viewsolicitudinspector->nombre_contacto->Visible) { // nombre_contacto ?>
	<div id="xsc_nombre_contacto" class="ewCell form-group">
		<label for="x_nombre_contacto" class="ewSearchCaption ewLabel"><?php echo $viewsolicitudinspector->nombre_contacto->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<input type="text" data-table="viewsolicitudinspector" data-field="x_nombre_contacto" name="x_nombre_contacto" id="x_nombre_contacto" size="30" maxlength="200" placeholder="<?php echo ew_HtmlEncode($viewsolicitudinspector->nombre_contacto->getPlaceHolder()) ?>" value="<?php echo $viewsolicitudinspector->nombre_contacto->EditValue ?>"<?php echo $viewsolicitudinspector->nombre_contacto->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_5" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($viewsolicitudinspector_list->BasicSearch->getKeyword()) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($viewsolicitudinspector_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $viewsolicitudinspector_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($viewsolicitudinspector_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($viewsolicitudinspector_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($viewsolicitudinspector_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($viewsolicitudinspector_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $viewsolicitudinspector_list->ShowPageHeader(); ?>
<?php
$viewsolicitudinspector_list->ShowMessage();
?>
<?php if ($viewsolicitudinspector_list->TotalRecs > 0 || $viewsolicitudinspector->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($viewsolicitudinspector_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> viewsolicitudinspector">
<form name="fviewsolicitudinspectorlist" id="fviewsolicitudinspectorlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($viewsolicitudinspector_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $viewsolicitudinspector_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="viewsolicitudinspector">
<div id="gmp_viewsolicitudinspector" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($viewsolicitudinspector_list->TotalRecs > 0 || $viewsolicitudinspector->CurrentAction == "gridedit") { ?>
<table id="tbl_viewsolicitudinspectorlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$viewsolicitudinspector_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$viewsolicitudinspector_list->RenderListOptions();

// Render list options (header, left)
$viewsolicitudinspector_list->ListOptions->Render("header", "left");
?>
<?php if ($viewsolicitudinspector->id->Visible) { // id ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->id) == "") { ?>
		<th data-name="id" class="<?php echo $viewsolicitudinspector->id->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_id" class="viewsolicitudinspector_id"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id" class="<?php echo $viewsolicitudinspector->id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->id) ?>',1);"><div id="elh_viewsolicitudinspector_id" class="viewsolicitudinspector_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->name->Visible) { // name ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->name) == "") { ?>
		<th data-name="name" class="<?php echo $viewsolicitudinspector->name->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_name" class="viewsolicitudinspector_name"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name" class="<?php echo $viewsolicitudinspector->name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->name) ?>',1);"><div id="elh_viewsolicitudinspector_name" class="viewsolicitudinspector_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->lastname->Visible) { // lastname ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->lastname) == "") { ?>
		<th data-name="lastname" class="<?php echo $viewsolicitudinspector->lastname->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_lastname" class="viewsolicitudinspector_lastname"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->lastname->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="lastname" class="<?php echo $viewsolicitudinspector->lastname->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->lastname) ?>',1);"><div id="elh_viewsolicitudinspector_lastname" class="viewsolicitudinspector_lastname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->lastname->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->lastname->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->lastname->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->_email->Visible) { // email ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->_email) == "") { ?>
		<th data-name="_email" class="<?php echo $viewsolicitudinspector->_email->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector__email" class="viewsolicitudinspector__email"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->_email->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_email" class="<?php echo $viewsolicitudinspector->_email->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->_email) ?>',1);"><div id="elh_viewsolicitudinspector__email" class="viewsolicitudinspector__email">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->_email->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->_email->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->_email->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->address->Visible) { // address ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->address) == "") { ?>
		<th data-name="address" class="<?php echo $viewsolicitudinspector->address->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_address" class="viewsolicitudinspector_address"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->address->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="address" class="<?php echo $viewsolicitudinspector->address->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->address) ?>',1);"><div id="elh_viewsolicitudinspector_address" class="viewsolicitudinspector_address">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->address->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->address->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->address->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->nombre_contacto->Visible) { // nombre_contacto ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->nombre_contacto) == "") { ?>
		<th data-name="nombre_contacto" class="<?php echo $viewsolicitudinspector->nombre_contacto->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_nombre_contacto" class="viewsolicitudinspector_nombre_contacto"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->nombre_contacto->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nombre_contacto" class="<?php echo $viewsolicitudinspector->nombre_contacto->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->nombre_contacto) ?>',1);"><div id="elh_viewsolicitudinspector_nombre_contacto" class="viewsolicitudinspector_nombre_contacto">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->nombre_contacto->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->nombre_contacto->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->nombre_contacto->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->email_contacto->Visible) { // email_contacto ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->email_contacto) == "") { ?>
		<th data-name="email_contacto" class="<?php echo $viewsolicitudinspector->email_contacto->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_email_contacto" class="viewsolicitudinspector_email_contacto"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->email_contacto->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="email_contacto" class="<?php echo $viewsolicitudinspector->email_contacto->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->email_contacto) ?>',1);"><div id="elh_viewsolicitudinspector_email_contacto" class="viewsolicitudinspector_email_contacto">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->email_contacto->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->email_contacto->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->email_contacto->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->phone->Visible) { // phone ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->phone) == "") { ?>
		<th data-name="phone" class="<?php echo $viewsolicitudinspector->phone->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_phone" class="viewsolicitudinspector_phone"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->phone->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="phone" class="<?php echo $viewsolicitudinspector->phone->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->phone) ?>',1);"><div id="elh_viewsolicitudinspector_phone" class="viewsolicitudinspector_phone">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->phone->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->phone->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->phone->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->cell->Visible) { // cell ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->cell) == "") { ?>
		<th data-name="cell" class="<?php echo $viewsolicitudinspector->cell->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_cell" class="viewsolicitudinspector_cell"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->cell->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="cell" class="<?php echo $viewsolicitudinspector->cell->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->cell) ?>',1);"><div id="elh_viewsolicitudinspector_cell" class="viewsolicitudinspector_cell">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->cell->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->cell->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->cell->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->tipoinmueble->Visible) { // tipoinmueble ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipoinmueble) == "") { ?>
		<th data-name="tipoinmueble" class="<?php echo $viewsolicitudinspector->tipoinmueble->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_tipoinmueble" class="viewsolicitudinspector_tipoinmueble"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipoinmueble->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tipoinmueble" class="<?php echo $viewsolicitudinspector->tipoinmueble->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipoinmueble) ?>',1);"><div id="elh_viewsolicitudinspector_tipoinmueble" class="viewsolicitudinspector_tipoinmueble">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipoinmueble->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->tipoinmueble->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->tipoinmueble->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->tipovehiculo->Visible) { // tipovehiculo ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipovehiculo) == "") { ?>
		<th data-name="tipovehiculo" class="<?php echo $viewsolicitudinspector->tipovehiculo->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_tipovehiculo" class="viewsolicitudinspector_tipovehiculo"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipovehiculo->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tipovehiculo" class="<?php echo $viewsolicitudinspector->tipovehiculo->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipovehiculo) ?>',1);"><div id="elh_viewsolicitudinspector_tipovehiculo" class="viewsolicitudinspector_tipovehiculo">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipovehiculo->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->tipovehiculo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->tipovehiculo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->tipomaquinaria->Visible) { // tipomaquinaria ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipomaquinaria) == "") { ?>
		<th data-name="tipomaquinaria" class="<?php echo $viewsolicitudinspector->tipomaquinaria->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_tipomaquinaria" class="viewsolicitudinspector_tipomaquinaria"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipomaquinaria->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tipomaquinaria" class="<?php echo $viewsolicitudinspector->tipomaquinaria->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipomaquinaria) ?>',1);"><div id="elh_viewsolicitudinspector_tipomaquinaria" class="viewsolicitudinspector_tipomaquinaria">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipomaquinaria->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->tipomaquinaria->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->tipomaquinaria->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->tipomercaderia->Visible) { // tipomercaderia ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipomercaderia) == "") { ?>
		<th data-name="tipomercaderia" class="<?php echo $viewsolicitudinspector->tipomercaderia->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_tipomercaderia" class="viewsolicitudinspector_tipomercaderia"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipomercaderia->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tipomercaderia" class="<?php echo $viewsolicitudinspector->tipomercaderia->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipomercaderia) ?>',1);"><div id="elh_viewsolicitudinspector_tipomercaderia" class="viewsolicitudinspector_tipomercaderia">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipomercaderia->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->tipomercaderia->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->tipomercaderia->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($viewsolicitudinspector->tipoespecial->Visible) { // tipoespecial ?>
	<?php if ($viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipoespecial) == "") { ?>
		<th data-name="tipoespecial" class="<?php echo $viewsolicitudinspector->tipoespecial->HeaderCellClass() ?>"><div id="elh_viewsolicitudinspector_tipoespecial" class="viewsolicitudinspector_tipoespecial"><div class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipoespecial->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tipoespecial" class="<?php echo $viewsolicitudinspector->tipoespecial->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $viewsolicitudinspector->SortUrl($viewsolicitudinspector->tipoespecial) ?>',1);"><div id="elh_viewsolicitudinspector_tipoespecial" class="viewsolicitudinspector_tipoespecial">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $viewsolicitudinspector->tipoespecial->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($viewsolicitudinspector->tipoespecial->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($viewsolicitudinspector->tipoespecial->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$viewsolicitudinspector_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($viewsolicitudinspector->ExportAll && $viewsolicitudinspector->Export <> "") {
	$viewsolicitudinspector_list->StopRec = $viewsolicitudinspector_list->TotalRecs;
} else {

	// Set the last record to display
	if ($viewsolicitudinspector_list->TotalRecs > $viewsolicitudinspector_list->StartRec + $viewsolicitudinspector_list->DisplayRecs - 1)
		$viewsolicitudinspector_list->StopRec = $viewsolicitudinspector_list->StartRec + $viewsolicitudinspector_list->DisplayRecs - 1;
	else
		$viewsolicitudinspector_list->StopRec = $viewsolicitudinspector_list->TotalRecs;
}
$viewsolicitudinspector_list->RecCnt = $viewsolicitudinspector_list->StartRec - 1;
if ($viewsolicitudinspector_list->Recordset && !$viewsolicitudinspector_list->Recordset->EOF) {
	$viewsolicitudinspector_list->Recordset->MoveFirst();
	$bSelectLimit = $viewsolicitudinspector_list->UseSelectLimit;
	if (!$bSelectLimit && $viewsolicitudinspector_list->StartRec > 1)
		$viewsolicitudinspector_list->Recordset->Move($viewsolicitudinspector_list->StartRec - 1);
} elseif (!$viewsolicitudinspector->AllowAddDeleteRow && $viewsolicitudinspector_list->StopRec == 0) {
	$viewsolicitudinspector_list->StopRec = $viewsolicitudinspector->GridAddRowCount;
}

// Initialize aggregate
$viewsolicitudinspector->RowType = EW_ROWTYPE_AGGREGATEINIT;
$viewsolicitudinspector->ResetAttrs();
$viewsolicitudinspector_list->RenderRow();
while ($viewsolicitudinspector_list->RecCnt < $viewsolicitudinspector_list->StopRec) {
	$viewsolicitudinspector_list->RecCnt++;
	if (intval($viewsolicitudinspector_list->RecCnt) >= intval($viewsolicitudinspector_list->StartRec)) {
		$viewsolicitudinspector_list->RowCnt++;

		// Set up key count
		$viewsolicitudinspector_list->KeyCount = $viewsolicitudinspector_list->RowIndex;

		// Init row class and style
		$viewsolicitudinspector->ResetAttrs();
		$viewsolicitudinspector->CssClass = "";
		if ($viewsolicitudinspector->CurrentAction == "gridadd") {
		} else {
			$viewsolicitudinspector_list->LoadRowValues($viewsolicitudinspector_list->Recordset); // Load row values
		}
		$viewsolicitudinspector->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$viewsolicitudinspector->RowAttrs = array_merge($viewsolicitudinspector->RowAttrs, array('data-rowindex'=>$viewsolicitudinspector_list->RowCnt, 'id'=>'r' . $viewsolicitudinspector_list->RowCnt . '_viewsolicitudinspector', 'data-rowtype'=>$viewsolicitudinspector->RowType));
		...
		$viewsolicitudinspector_list->RenderListOptions();
?>
	<tr<?php echo $viewsolicitudinspector->RowAttributes() ?>>
<?php

// Render list options (body, left)
$viewsolicitudinspector_list->ListOptions->Render("body", "left", $viewsolicitudinspector_list->RowCnt);
?>
	<?php if ($viewsolicitudinspector->id->Visible) { // id ?>
		<td data-name="id"<?php echo $viewsolicitudinspector->id->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_id" class="viewsolicitudinspector_id">
...
<?php echo $viewsolicitudinspector->id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->name->Visible) { // name ?>
		<td data-name="name"<?php echo $viewsolicitudinspector->name->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_name" class="viewsolicitudinspector_name">
...
<?php echo $viewsolicitudinspector->name->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->lastname->Visible) { // lastname ?>
		<td data-name="lastname"<?php echo $viewsolicitudinspector->lastname->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_lastname" class="viewsolicitudinspector_lastname">
...
<?php echo $viewsolicitudinspector->lastname->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->_email->Visible) { // email ?>
		<td data-name="_email"<?php echo $viewsolicitudinspector->_email->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector__email" class="viewsolicitudinspector__email">
...
<?php echo $viewsolicitudinspector->_email->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->address->Visible) { // address ?>
		<td data-name="address"<?php echo $viewsolicitudinspector->address->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_address" class="viewsolicitudinspector_address">
...
<?php echo $viewsolicitudinspector->address->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->nombre_contacto->Visible) { // nombre_contacto ?>
		<td data-name="nombre_contacto"<?php echo $viewsolicitudinspector->nombre_contacto->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_nombre_contacto" class="viewsolicitudinspector_nombre_contacto">
...
<?php echo $viewsolicitudinspector->nombre_contacto->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->email_contacto->Visible) { // email_contacto ?>
		<td data-name="email_contacto"<?php echo $viewsolicitudinspector->email_contacto->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_email_contacto" class="viewsolicitudinspector_email_contacto">
...
<?php echo $viewsolicitudinspector->email_contacto->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->phone->Visible) { // phone ?>
		<td data-name="phone"<?php echo $viewsolicitudinspector->phone->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_phone" class="viewsolicitudinspector_phone">
...
<?php echo $viewsolicitudinspector->phone->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->cell->Visible) { // cell ?>
		<td data-name="cell"<?php echo $viewsolicitudinspector->cell->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_cell" class="viewsolicitudinspector_cell">
...
<?php echo $viewsolicitudinspector->cell->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->tipoinmueble->Visible) { // tipoinmueble ?>
		<td data-name="tipoinmueble"<?php echo $viewsolicitudinspector->tipoinmueble->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_tipoinmueble" class="viewsolicitudinspector_tipoinmueble">
...
<?php echo $viewsolicitudinspector->tipoinmueble->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->tipovehiculo->Visible) { // tipovehiculo ?>
		<td data-name="tipovehiculo"<?php echo $viewsolicitudinspector->tipovehiculo->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_tipovehiculo" class="viewsolicitudinspector_tipovehiculo">
...
<?php echo $viewsolicitudinspector->tipovehiculo->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->tipomaquinaria->Visible) { // tipomaquinaria ?>
		<td data-name="tipomaquinaria"<?php echo $viewsolicitudinspector->tipomaquinaria->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_tipomaquinaria" class="viewsolicitudinspector_tipomaquinaria">
...
<?php echo $viewsolicitudinspector->tipomaquinaria->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->tipomercaderia->Visible) { // tipomercaderia ?>
		<td data-name="tipomercaderia"<?php echo $viewsolicitudinspector->tipomercaderia->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_tipomercaderia" class="viewsolicitudinspector_tipomercaderia">
...
<?php echo $viewsolicitudinspector->tipomercaderia->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($viewsolicitudinspector->tipoespecial->Visible) { // tipoespecial ?>
		<td data-name="tipoespecial"<?php echo $viewsolicitudinspector->tipoespecial->CellAttributes() ?>>
<span id="el<?php echo $viewsolicitudinspector_list->RowCnt ?>_viewsolicitudinspector_tipoespecial" class="viewsolicitudinspector_tipoespecial">
...
$viewsolicitudinspector_list->ListOptions->Render("body", "right", $viewsolicitudinspector_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($viewsolicitudinspector->CurrentAction <> "gridadd")
		$viewsolicitudinspector_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($viewsolicitudinspector->CurrentAction == "") { ?>
...
// Close recordset
if ($viewsolicitudinspector_list->Recordset)
	$viewsolicitudinspector_list->Recordset->Close();
?>
<?php if ($viewsolicitudinspector->Export == "") { ?>
<div class="box-footer ewGridLowerPanel">
<?php if ($viewsolicitudinspector->CurrentAction <> "gridadd" && $viewsolicitudinspector->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($viewsolicitudinspector_list->Pager)) $viewsolicitudinspector_list->Pager = new cNumericPager($viewsolicitudinspector_list->StartRec, $viewsolicitudinspector_list->DisplayRecs, $viewsolicitudinspector_list->TotalRecs, $viewsolicitudinspector_list->RecRange, $viewsolicitudinspector_list->AutoHidePager) ?>
<?php if ($viewsolicitudinspector_list->Pager->RecordCount > 0 && $viewsolicitudinspector_list->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($viewsolicitudinspector_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $viewsolicitudinspector_list->PageUrl() ?>start=<?php echo $viewsolicitudinspector_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($viewsolicitudinspector_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $viewsolicitudinspector_list->PageUrl() ?>start=<?php echo $viewsolicitudinspector_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($viewsolicitudinspector_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $viewsolicitudinspector_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($viewsolicitudinspector_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $viewsolicitudinspector_list->PageUrl() ?>start=<?php echo $viewsolicitudinspector_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($viewsolicitudinspector_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $viewsolicitudinspector_list->PageUrl() ?>start=<?php echo $viewsolicitudinspector_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
<?php if ($viewsolicitudinspector_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $viewsolicitudinspector_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $viewsolicitudinspector_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $viewsolicitudinspector_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($viewsolicitudinspector_list->TotalRecs > 0 && (!$viewsolicitudinspector_list->AutoHidePageSizeSelector || $viewsolicitudinspector_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="viewsolicitudinspector">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($viewsolicitudinspector_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($viewsolicitudinspector_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($viewsolicitudinspector_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($viewsolicitudinspector->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($viewsolicitudinspector_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($viewsolicitudinspector_list->TotalRecs == 0 && $viewsolicitudinspector->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($viewsolicitudinspector_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($viewsolicitudinspector->Export == "") { ?>
<script type="text/javascript">
fviewsolicitudinspectorlistsrch.FilterList = <?php echo $viewsolicitudinspector_list->GetFilterList() ?>;
fviewsolicitudinspectorlistsrch.Init();
fviewsolicitudinspectorlist.Init();
</script>
<?php } ?>
<?php
$viewsolicitudinspector_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($viewsolicitudinspector->Export == "") { ?>
...
<?php
$viewsolicitudinspector_list->Page_Terminate();
?>
