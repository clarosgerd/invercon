...
$historico_delete = NULL; // Initialize page object first

class chistorico_delete extends chistorico {
	...
	// Page object name
	var $PageObjName = 'historico_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("historicolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in historico class, historicoinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("historicolist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($historico_delete)) $historico_delete = new chistorico_delete();

// Page init
$historico_delete->Page_Init();

// Page main
$historico_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$historico_delete->Page_Render();
?>
...
var CurrentForm = fhistoricodelete = new ew_Form("fhistoricodelete", "delete");

// Form_CustomValidate event
fhistoricodelete.Form_CustomValidate = 
...
fhistoricodelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fhistoricodelete.Lists["x_proceso"] = {"LinkField":"x_descripcion","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estadointerno"};
fhistoricodelete.Lists["x_proceso"].Data = "<?php echo $historico_delete->proceso->LookupFilterQuery(FALSE, "delete") ?>";
fhistoricodelete.Lists["x_responsable"] = {"LinkField":"x__login","Ajax":true,"AutoFill":false,"DisplayFields":["x_codigo","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"usuario"};
fhistoricodelete.Lists["x_responsable"].Data = "<?php echo $historico_delete->responsable->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $historico_delete->ShowPageHeader(); ?>
<?php
$historico_delete->ShowMessage();
?>
<form name="fhistoricodelete" id="fhistoricodelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($historico_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $historico_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="historico">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($historico_delete->RecKeys as $key) { ?>
...
<?php if ($historico->id->Visible) { // id ?>
		<th class="<?php echo $historico->id->HeaderCellClass() ?>"><span id="elh_historico_id" class="historico_id"><?php echo $historico->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($historico->proceso->Visible) { // proceso ?>
		<th class="<?php echo $historico->proceso->HeaderCellClass() ?>"><span id="elh_historico_proceso" class="historico_proceso"><?php echo $historico->proceso->FldCaption() ?></span></th>
<?php } ?>
<?php if ($historico->responsable->Visible) { // responsable ?>
		<th class="<?php echo $historico->responsable->HeaderCellClass() ?>"><span id="elh_historico_responsable" class="historico_responsable"><?php echo $historico->responsable->FldCaption() ?></span></th>
<?php } ?>
<?php if ($historico->ingreso->Visible) { // ingreso ?>
		<th class="<?php echo $historico->ingreso->HeaderCellClass() ?>"><span id="elh_historico_ingreso" class="historico_ingreso"><?php echo $historico->ingreso->FldCaption() ?></span></th>
<?php } ?>
<?php if ($historico->salida->Visible) { // salida ?>
		<th class="<?php echo $historico->salida->HeaderCellClass() ?>"><span id="elh_historico_salida" class="historico_salida"><?php echo $historico->salida->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$historico_delete->RecCnt = 0;
$i = 0;
while (!$historico_delete->Recordset->EOF) {
	$historico_delete->RecCnt++;
	$historico_delete->RowCnt++;

	// Set row properties
	$historico->ResetAttrs();
	$historico->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$historico_delete->LoadRowValues($historico_delete->Recordset);

	// Render row
	$historico_delete->RenderRow();
?>
	<tr<?php echo $historico->RowAttributes() ?>>
<?php if ($historico->id->Visible) { // id ?>
		<td<?php echo $historico->id->CellAttributes() ?>>
<span id="el<?php echo $historico_delete->RowCnt ?>_historico_id" class="historico_id">
...
<span id="el<?php echo $historico_delete->RowCnt ?>_historico_proceso" class="historico_proceso">
...
<span id="el<?php echo $historico_delete->RowCnt ?>_historico_responsable" class="historico_responsable">
...
<span id="el<?php echo $historico_delete->RowCnt ?>_historico_ingreso" class="historico_ingreso">
...
<span id="el<?php echo $historico_delete->RowCnt ?>_historico_salida" class="historico_salida">
...
<?php
	$historico_delete->Recordset->MoveNext();
}
$historico_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $historico_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fhistoricodelete.Init();
</script>
<?php
$historico_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$historico_delete->Page_Terminate();
?>
