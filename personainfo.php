...
class cpersona extends cTable {
	...
	function __construct() {
		...
		// name
		$this->name = new cField('persona', 'persona', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// lastname
		$this->lastname = new cField('persona', 'persona', 'x_lastname', 'lastname', '`lastname`', '`lastname`', 200, -1, FALSE, '`lastname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->lastname->Sortable = TRUE; // Allow sort
		$this->fields['lastname'] = &$this->lastname;

		// day_of_birth
		$this->day_of_birth = new cField('persona', 'persona', 'x_day_of_birth', 'day_of_birth', '`day_of_birth`', ew_CastDateFieldForLike('`day_of_birth`', 0, "DB"), 133, 0, FALSE, '`day_of_birth`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// address
		$this->address = new cField('persona', 'persona', 'x_address', 'address', '`address`', '`address`', 200, -1, FALSE, '`address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->address->Sortable = TRUE; // Allow sort
		$this->fields['address'] = &$this->address;

		// phone
		$this->phone = new cField('persona', 'persona', 'x_phone', 'phone', '`phone`', '`phone`', 200, -1, FALSE, '`phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->phone->Sortable = TRUE; // Allow sort
		$this->fields['phone'] = &$this->phone;

		// id_tipo
		$this->id_tipo = new cField('persona', 'persona', 'x_id_tipo', 'id_tipo', '`id_tipo`', '`id_tipo`', 3, -1, FALSE, '`id_tipo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_tipo->Sortable = TRUE; // Allow sort
		$this->id_tipo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_tipo'] = &$this->id_tipo;

		// is_active
		$this->is_active = new cField('persona', 'persona', 'x_is_active', 'is_active', '`is_active`', '`is_active`', 16, -1, FALSE, '`is_active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// created_at
		$this->created_at = new cField('persona', 'persona', 'x_created_at', 'created_at', '`created_at`', ew_CastDateFieldForLike('`created_at`', 0, "DB"), 135, 0, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->Sortable = TRUE; // Allow sort
		$this->created_at->FldDefaultErrMsg = str_replace("%s", $GLOBALS["EW_DATE_FORMAT"], $Language->Phrase("IncorrectDate"));
		$this->fields['created_at'] = &$this->created_at;

		// category_id
		$this->category_id = new cField('persona', 'persona', 'x_category_id', 'category_id', '`category_id`', '`category_id`', 3, -1, FALSE, '`category_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->id_tipo->Exportable) $Doc->ExportCaption($this->id_tipo);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->id_tipo->Exportable) $Doc->ExportField($this->id_tipo);
