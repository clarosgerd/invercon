...
class ccliente_list extends ccliente {
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fclientelistsrch");
	...
	function SetupListOptions() {
		...
		// "detail_pago"
		$item = &$this->ListOptions->Add("detail_pago");
		$item->CssClass = "text-nowrap";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'pago') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_pago"
		$oListOpt = &$this->ListOptions->Items["detail_pago"];
		if ($Security->AllowList(CurrentProjectID() . 'pago')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("pago", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("pagolist.php?" . EW_TABLE_SHOW_MASTER . "=cliente&fk_id=" . urlencode(strval($this->id->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["pago_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'pago')) {
				$caption = $Language->Phrase("MasterDetailViewLink");
				$url = $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=pago");
				...
				$DetailViewTblVar .= "pago";
			}
			if ($GLOBALS["pago_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'pago')) {
				$caption = $Language->Phrase("MasterDetailEditLink");
				$url = $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=pago");
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_pago");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=pago");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["pago"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["pago"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'pago') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "pago";
		}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fclientelistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
...
fclientelist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($cliente_list->TotalRecs <= 0)
			$cliente_list->TotalRecs = $cliente->ListRecordCount();
	} else {
...
<form name="fclientelistsrch" id="fclientelistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
...
<div id="xsr_1" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($cliente_list->BasicSearch->getKeyword()) ?>">
...
<div class="box ewBox ewGrid<?php if ($cliente_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> cliente">
...
<?php } ?>
<input type="hidden" name="t" value="cliente">
<div id="gmp_cliente" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($cliente_list->TotalRecs > 0 || $cliente->CurrentAction == "gridedit") { ?>
...
<?php if ($cliente->id->Visible) { // id ?>
	<?php if ($cliente->SortUrl($cliente->id) == "") { ?>
		<th data-name="id" class="<?php echo $cliente->id->HeaderCellClass() ?>"><div id="elh_cliente_id" class="cliente_id"><div class="ewTableHeaderCaption"><?php echo $cliente->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id" class="<?php echo $cliente->id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->id) ?>',1);"><div id="elh_cliente_id" class="cliente_id">
...
<?php if ($cliente->name->Visible) { // name ?>
	<?php if ($cliente->SortUrl($cliente->name) == "") { ?>
		<th data-name="name" class="<?php echo $cliente->name->HeaderCellClass() ?>"><div id="elh_cliente_name" class="cliente_name"><div class="ewTableHeaderCaption"><?php echo $cliente->name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name" class="<?php echo $cliente->name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->name) ?>',1);"><div id="elh_cliente_name" class="cliente_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cliente->name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cliente->lastname->Visible) { // lastname ?>
	<?php if ($cliente->SortUrl($cliente->lastname) == "") { ?>
		<th data-name="lastname" class="<?php echo $cliente->lastname->HeaderCellClass() ?>"><div id="elh_cliente_lastname" class="cliente_lastname"><div class="ewTableHeaderCaption"><?php echo $cliente->lastname->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="lastname" class="<?php echo $cliente->lastname->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->lastname) ?>',1);"><div id="elh_cliente_lastname" class="cliente_lastname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->lastname->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cliente->lastname->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->lastname->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cliente->_email->Visible) { // email ?>
	<?php if ($cliente->SortUrl($cliente->_email) == "") { ?>
		<th data-name="_email" class="<?php echo $cliente->_email->HeaderCellClass() ?>"><div id="elh_cliente__email" class="cliente__email"><div class="ewTableHeaderCaption"><?php echo $cliente->_email->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_email" class="<?php echo $cliente->_email->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->_email) ?>',1);"><div id="elh_cliente__email" class="cliente__email">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->_email->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cliente->_email->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->_email->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cliente->address->Visible) { // address ?>
	<?php if ($cliente->SortUrl($cliente->address) == "") { ?>
		<th data-name="address" class="<?php echo $cliente->address->HeaderCellClass() ?>"><div id="elh_cliente_address" class="cliente_address"><div class="ewTableHeaderCaption"><?php echo $cliente->address->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="address" class="<?php echo $cliente->address->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->address) ?>',1);"><div id="elh_cliente_address" class="cliente_address">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->address->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cliente->address->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->address->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cliente->phone->Visible) { // phone ?>
	<?php if ($cliente->SortUrl($cliente->phone) == "") { ?>
		<th data-name="phone" class="<?php echo $cliente->phone->HeaderCellClass() ?>"><div id="elh_cliente_phone" class="cliente_phone"><div class="ewTableHeaderCaption"><?php echo $cliente->phone->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="phone" class="<?php echo $cliente->phone->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->phone) ?>',1);"><div id="elh_cliente_phone" class="cliente_phone">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->phone->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cliente->phone->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->phone->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cliente->phone_cell->Visible) { // phone_cell ?>
	<?php if ($cliente->SortUrl($cliente->phone_cell) == "") { ?>
		<th data-name="phone_cell" class="<?php echo $cliente->phone_cell->HeaderCellClass() ?>"><div id="elh_cliente_phone_cell" class="cliente_phone_cell"><div class="ewTableHeaderCaption"><?php echo $cliente->phone_cell->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="phone_cell" class="<?php echo $cliente->phone_cell->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->phone_cell) ?>',1);"><div id="elh_cliente_phone_cell" class="cliente_phone_cell">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->phone_cell->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($cliente->phone_cell->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->phone_cell->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($cliente->is_active->Visible) { // is_active ?>
	<?php if ($cliente->SortUrl($cliente->is_active) == "") { ?>
		<th data-name="is_active" class="<?php echo $cliente->is_active->HeaderCellClass() ?>"><div id="elh_cliente_is_active" class="cliente_is_active"><div class="ewTableHeaderCaption"><?php echo $cliente->is_active->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="is_active" class="<?php echo $cliente->is_active->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $cliente->SortUrl($cliente->is_active) ?>',1);"><div id="elh_cliente_is_active" class="cliente_is_active">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $cliente->is_active->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($cliente->is_active->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($cliente->is_active->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_id" class="cliente_id">
...
<?php echo $cliente->id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($cliente->name->Visible) { // name ?>
		<td data-name="name"<?php echo $cliente->name->CellAttributes() ?>>
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_name" class="cliente_name">
...
<?php echo $cliente->name->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($cliente->lastname->Visible) { // lastname ?>
		<td data-name="lastname"<?php echo $cliente->lastname->CellAttributes() ?>>
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_lastname" class="cliente_lastname">
...
<?php echo $cliente->_email->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($cliente->address->Visible) { // address ?>
		<td data-name="address"<?php echo $cliente->address->CellAttributes() ?>>
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_address" class="cliente_address">
...
<?php echo $cliente->address->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($cliente->phone->Visible) { // phone ?>
		<td data-name="phone"<?php echo $cliente->phone->CellAttributes() ?>>
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_phone" class="cliente_phone">
...
<?php echo $cliente->phone->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($cliente->phone_cell->Visible) { // phone_cell ?>
		<td data-name="phone_cell"<?php echo $cliente->phone_cell->CellAttributes() ?>>
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_phone_cell" class="cliente_phone_cell">
...
<?php echo $cliente->phone_cell->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($cliente->is_active->Visible) { // is_active ?>
		<td data-name="is_active"<?php echo $cliente->is_active->CellAttributes() ?>>
<span id="el<?php echo $cliente_list->RowCnt ?>_cliente_is_active" class="cliente_is_active">
...
<?php if (!isset($cliente_list->Pager)) $cliente_list->Pager = new cNumericPager($cliente_list->StartRec, $cliente_list->DisplayRecs, $cliente_list->TotalRecs, $cliente_list->RecRange, $cliente_list->AutoHidePager) ?>
<?php if ($cliente_list->Pager->RecordCount > 0 && $cliente_list->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($cliente_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_list->PageUrl() ?>start=<?php echo $cliente_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($cliente_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_list->PageUrl() ?>start=<?php echo $cliente_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($cliente_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $cliente_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($cliente_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_list->PageUrl() ?>start=<?php echo $cliente_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($cliente_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $cliente_list->PageUrl() ?>start=<?php echo $cliente_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<?php if ($cliente_list->TotalRecs > 0 && (!$cliente_list->AutoHidePageSizeSelector || $cliente_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="cliente">
