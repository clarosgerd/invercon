...
$comentariosavaluo_delete = NULL; // Initialize page object first

class ccomentariosavaluo_delete extends ccomentariosavaluo {
	...
	// Page object name
	var $PageObjName = 'comentariosavaluo_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("comentariosavaluolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in comentariosavaluo class, comentariosavaluoinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("comentariosavaluolist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($comentariosavaluo_delete)) $comentariosavaluo_delete = new ccomentariosavaluo_delete();

// Page init
$comentariosavaluo_delete->Page_Init();

// Page main
$comentariosavaluo_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$comentariosavaluo_delete->Page_Render();
?>
...
var CurrentForm = fcomentariosavaluodelete = new ew_Form("fcomentariosavaluodelete", "delete");

// Form_CustomValidate event
fcomentariosavaluodelete.Form_CustomValidate = 
...
fcomentariosavaluodelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $comentariosavaluo_delete->ShowPageHeader(); ?>
<?php
$comentariosavaluo_delete->ShowMessage();
?>
<form name="fcomentariosavaluodelete" id="fcomentariosavaluodelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($comentariosavaluo_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $comentariosavaluo_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="comentariosavaluo">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($comentariosavaluo_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$comentariosavaluo_delete->RecCnt = 0;
$i = 0;
while (!$comentariosavaluo_delete->Recordset->EOF) {
	$comentariosavaluo_delete->RecCnt++;
	$comentariosavaluo_delete->RowCnt++;

	// Set row properties
	$comentariosavaluo->ResetAttrs();
	$comentariosavaluo->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$comentariosavaluo_delete->LoadRowValues($comentariosavaluo_delete->Recordset);

	// Render row
	$comentariosavaluo_delete->RenderRow();
?>
	<tr<?php echo $comentariosavaluo->RowAttributes() ?>>
<?php if ($comentariosavaluo->id->Visible) { // id ?>
		<td<?php echo $comentariosavaluo->id->CellAttributes() ?>>
<span id="el<?php echo $comentariosavaluo_delete->RowCnt ?>_comentariosavaluo_id" class="comentariosavaluo_id">
...
<?php
	$comentariosavaluo_delete->Recordset->MoveNext();
}
$comentariosavaluo_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $comentariosavaluo_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcomentariosavaluodelete.Init();
</script>
<?php
$comentariosavaluo_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$comentariosavaluo_delete->Page_Terminate();
?>
