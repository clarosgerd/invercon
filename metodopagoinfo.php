...
//
// Table class for metodopago
//
class cmetodopago extends cTable {
	var $id;
	var $short_name;
	var $name;
	var $is_active;
	...
	function __construct() {
		...
		$this->TableVar = 'metodopago';
		$this->TableName = 'metodopago';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`metodopago`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('metodopago', 'metodopago', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// short_name
		$this->short_name = new cField('metodopago', 'metodopago', 'x_short_name', 'short_name', '`short_name`', '`short_name`', 200, -1, FALSE, '`short_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->short_name->Sortable = TRUE; // Allow sort
		$this->fields['short_name'] = &$this->short_name;

		// name
		$this->name = new cField('metodopago', 'metodopago', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateModified
		$this->DateModified = new cField('metodopago', 'metodopago', 'x_DateModified', 'DateModified', '`DateModified`', ew_CastDateFieldForLike('`DateModified`', 0, "DB"), 135, 0, FALSE, '`DateModified`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateDeleted
		$this->DateDeleted = new cField('metodopago', 'metodopago', 'x_DateDeleted', 'DateDeleted', '`DateDeleted`', ew_CastDateFieldForLike('`DateDeleted`', 0, "DB"), 135, 0, FALSE, '`DateDeleted`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// CreatedBy
		$this->CreatedBy = new cField('metodopago', 'metodopago', 'x_CreatedBy', 'CreatedBy', '`CreatedBy`', '`CreatedBy`', 200, -1, FALSE, '`CreatedBy`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreatedBy->Sortable = FALSE; // Allow sort
		$this->fields['CreatedBy'] = &$this->CreatedBy;

		// ModifiedBy
		$this->ModifiedBy = new cField('metodopago', 'metodopago', 'x_ModifiedBy', 'ModifiedBy', '`ModifiedBy`', '`ModifiedBy`', 200, -1, FALSE, '`ModifiedBy`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ModifiedBy->Sortable = FALSE; // Allow sort
		$this->fields['ModifiedBy'] = &$this->ModifiedBy;

		// DeletedBy
		$this->DeletedBy = new cField('metodopago', 'metodopago', 'x_DeletedBy', 'DeletedBy', '`DeletedBy`', '`DeletedBy`', 200, -1, FALSE, '`DeletedBy`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`metodopago`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "metodopagoedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "metodopagoadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("metodopagoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("metodopagoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "metodopagoadd.php?" . $this->UrlParm($parm);
		else
			$url = "metodopagoadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("metodopagoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("metodopagoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("metodopagodelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->short_name->setDbValue($rs->fields('short_name'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->short_name->Exportable) $Doc->ExportCaption($this->short_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->short_name->Exportable) $Doc->ExportField($this->short_name);
