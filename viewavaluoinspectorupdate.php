...
$viewavaluoinspector_update = NULL; // Initialize page object first

class cviewavaluoinspector_update extends cviewavaluoinspector {
	...
	// Page object name
	var $PageObjName = 'viewavaluoinspector_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("viewavaluoinspectorlist.php"); // No records selected, return to list
...
// Create page object
if (!isset($viewavaluoinspector_update)) $viewavaluoinspector_update = new cviewavaluoinspector_update();

// Page init
$viewavaluoinspector_update->Page_Init();

// Page main
$viewavaluoinspector_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$viewavaluoinspector_update->Page_Render();
?>
...
var CurrentForm = fviewavaluoinspectorupdate = new ew_Form("fviewavaluoinspectorupdate", "update");

// Validate form
fviewavaluoinspectorupdate.Validate = function() {
...
fviewavaluoinspectorupdate.Form_CustomValidate = 
...
fviewavaluoinspectorupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fviewavaluoinspectorupdate.Lists["x_estadointerno"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"estadointerno"};
fviewavaluoinspectorupdate.Lists["x_estadointerno"].Data = "<?php echo $viewavaluoinspector_update->estadointerno->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $viewavaluoinspector_update->ShowPageHeader(); ?>
<?php
$viewavaluoinspector_update->ShowMessage();
?>
<form name="fviewavaluoinspectorupdate" id="fviewavaluoinspectorupdate" class="<?php echo $viewavaluoinspector_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($viewavaluoinspector_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $viewavaluoinspector_update->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="viewavaluoinspector">
<input type="hidden" name="a_update" id="a_update" value="U">
<input type="hidden" name="modal" value="<?php echo intval($viewavaluoinspector_update->IsModal) ?>">
<?php foreach ($viewavaluoinspector_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<?php if (!$viewavaluoinspector_update->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($viewavaluoinspector_update->IsMobileOrModal) { ?>
<div id="tbl_viewavaluoinspectorupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php } else { ?>
<table id="tbl_viewavaluoinspectorupdate" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- desktop table -->
	<thead>
	<tr>
...
<?php if ($viewavaluoinspector_update->IsMobileOrModal) { ?>
	<div id="r_estadointerno" class="form-group">
		<label for="x_estadointerno" class="<?php echo $viewavaluoinspector_update->LeftColumnClass ?>"><div class="checkbox"><label>
<input type="checkbox" name="u_estadointerno" id="u_estadointerno" class="ewMultiSelect" value="1"<?php echo ($viewavaluoinspector->estadointerno->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $viewavaluoinspector->estadointerno->FldCaption() ?></label></div></label>
		<div class="<?php echo $viewavaluoinspector_update->RightColumnClass ?>"><div<?php echo $viewavaluoinspector->estadointerno->CellAttributes() ?>>
<span id="el_viewavaluoinspector_estadointerno">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next(":not([disabled])").click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_estadointerno"><?php echo (strval($viewavaluoinspector->estadointerno->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $viewavaluoinspector->estadointerno->ViewValue); ?></span>
</span>
...
<?php } else { ?>
	<tr id="r_estadointerno">
		<td class="col-sm-3"<?php echo $viewavaluoinspector->estadointerno->CellAttributes() ?>><span id="elh_viewavaluoinspector_estadointerno"><div class="checkbox"><label for="u_estadointerno">
<input type="checkbox" name="u_estadointerno" id="u_estadointerno" class="ewMultiSelect" value="1"<?php echo ($viewavaluoinspector->estadointerno->MultiUpdate == "1") ? " checked" : "" ?>>
 <?php echo $viewavaluoinspector->estadointerno->FldCaption() ?></label></div></span></td>
...
<?php } ?>
<?php if (!$viewavaluoinspector_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $viewavaluoinspector_update->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("UpdateBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $viewavaluoinspector_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
		</div><!-- /buttons offset -->
...
<script type="text/javascript">
fviewavaluoinspectorupdate.Init();
</script>
<?php
$viewavaluoinspector_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$viewavaluoinspector_update->Page_Terminate();
?>
