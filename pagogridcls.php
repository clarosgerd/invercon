...
$pago_grid = NULL; // Initialize page object first

class cpago_grid extends cpago {
	...
	// Page object name
	var $PageObjName = 'pago_grid';

	// Grid form hidden field names
	var $FormName = 'fpagogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["pago"];

		}
		$this->AddUrl = "pagoadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_code") && $objForm->HasValue("o_code") && $this->code->CurrentValue <> $this->code->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_cliente_id") && $objForm->HasValue("o_cliente_id") && $this->cliente_id->CurrentValue <> $this->cliente_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_status_id") && $objForm->HasValue("o_status_id") && $this->status_id->CurrentValue <> $this->status_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_metodopago_id") && $objForm->HasValue("o_metodopago_id") && $this->metodopago_id->CurrentValue <> $this->metodopago_id->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		...
		$this->documento_pago->Upload->Index = $this->RowIndex;
	}
	...
	function LoadFormValues() {
		...
		$this->cliente_id->setOldValue($objForm->GetValue("o_cliente_id"));
		...
		$this->metodopago_id->setOldValue($objForm->GetValue("o_metodopago_id"));
	}
