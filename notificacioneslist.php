...
$notificaciones_list = NULL; // Initialize page object first

class cnotificaciones_list extends cnotificaciones {
	...
	// Page object name
	var $PageObjName = 'notificaciones_list';

	// Grid form hidden field names
	var $FormName = 'fnotificacioneslist';
	...
	function __construct() {
		...
		$this->AddUrl = "notificacionesadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "notificacionesdelete.php";
		$this->MultiUpdateUrl = "notificacionesupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fnotificacioneslistsrch";
	...
	function RestoreFilterList() {
		...
		// Field creadopor
		$this->creadopor->AdvancedSearch->SearchValue = @$filter["x_creadopor"];
		$this->creadopor->AdvancedSearch->SearchOperator = @$filter["z_creadopor"];
		$this->creadopor->AdvancedSearch->SearchCondition = @$filter["v_creadopor"];
		$this->creadopor->AdvancedSearch->SearchValue2 = @$filter["y_creadopor"];
		$this->creadopor->AdvancedSearch->SearchOperator2 = @$filter["w_creadopor"];
		$this->creadopor->AdvancedSearch->Save();

		// Field recibidopor
		$this->recibidopor->AdvancedSearch->SearchValue = @$filter["x_recibidopor"];
		$this->recibidopor->AdvancedSearch->SearchOperator = @$filter["z_recibidopor"];
		$this->recibidopor->AdvancedSearch->SearchCondition = @$filter["v_recibidopor"];
		$this->recibidopor->AdvancedSearch->SearchValue2 = @$filter["y_recibidopor"];
		$this->recibidopor->AdvancedSearch->SearchOperator2 = @$filter["w_recibidopor"];
		$this->recibidopor->AdvancedSearch->Save();
		...
		// Field desde
		$this->desde->AdvancedSearch->SearchValue = @$filter["x_desde"];
		$this->desde->AdvancedSearch->SearchOperator = @$filter["z_desde"];
		$this->desde->AdvancedSearch->SearchCondition = @$filter["v_desde"];
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->creadopor, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->recibidopor, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->desde, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fnotificacioneslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fnotificacioneslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fnotificacioneslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fnotificacioneslistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_notificaciones\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_notificaciones',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fnotificacioneslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($notificaciones_list)) $notificaciones_list = new cnotificaciones_list();

// Page init
$notificaciones_list->Page_Init();

// Page main
$notificaciones_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$notificaciones_list->Page_Render();
?>
...
var CurrentForm = fnotificacioneslist = new ew_Form("fnotificacioneslist", "list");
fnotificacioneslist.FormKeyCountName = '<?php echo $notificaciones_list->FormKeyCountName ?>';

// Form_CustomValidate event
fnotificacioneslist.Form_CustomValidate = 
...
fnotificacioneslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fnotificacioneslistsrch = new ew_Form("fnotificacioneslistsrch");
</script>
<script type="text/javascript">
...
<?php if ($notificaciones->Export == "") { ?>
<div class="ewToolbar">
<?php if ($notificaciones_list->TotalRecs > 0 && $notificaciones_list->ExportOptions->Visible()) { ?>
<?php $notificaciones_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($notificaciones_list->SearchOptions->Visible()) { ?>
<?php $notificaciones_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($notificaciones_list->FilterOptions->Visible()) { ?>
<?php $notificaciones_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $notificaciones_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($notificaciones_list->TotalRecs <= 0)
			$notificaciones_list->TotalRecs = $notificaciones->ListRecordCount();
	} else {
		if (!$notificaciones_list->Recordset && ($notificaciones_list->Recordset = $notificaciones_list->LoadRecordset()))
			$notificaciones_list->TotalRecs = $notificaciones_list->Recordset->RecordCount();
	}
	$notificaciones_list->StartRec = 1;
	if ($notificaciones_list->DisplayRecs <= 0 || ($notificaciones->Export <> "" && $notificaciones->ExportAll)) // Display all records
		$notificaciones_list->DisplayRecs = $notificaciones_list->TotalRecs;
	if (!($notificaciones->Export <> "" && $notificaciones->ExportAll))
		$notificaciones_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$notificaciones_list->Recordset = $notificaciones_list->LoadRecordset($notificaciones_list->StartRec-1, $notificaciones_list->DisplayRecs);

	// Set no record found message
	if ($notificaciones->CurrentAction == "" && $notificaciones_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$notificaciones_list->setWarningMessage(ew_DeniedMsg());
		if ($notificaciones_list->SearchWhere == "0=101")
			$notificaciones_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$notificaciones_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$notificaciones_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($notificaciones->Export == "" && $notificaciones->CurrentAction == "") { ?>
<form name="fnotificacioneslistsrch" id="fnotificacioneslistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($notificaciones_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fnotificacioneslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="notificaciones">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($notificaciones_list->BasicSearch->getKeyword()) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($notificaciones_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $notificaciones_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($notificaciones_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($notificaciones_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($notificaciones_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($notificaciones_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $notificaciones_list->ShowPageHeader(); ?>
<?php
...
if ($notificaciones->ExportAll && $notificaciones->Export <> "") {
	...
	// Set the last record to display
	if ($notificaciones_list->TotalRecs > $notificaciones_list->StartRec + $notificaciones_list->DisplayRecs - 1)
		$notificaciones_list->StopRec = $notificaciones_list->StartRec + $notificaciones_list->DisplayRecs - 1;
	else
		$notificaciones_list->StopRec = $notificaciones_list->TotalRecs;
}
$notificaciones_list->RecCnt = $notificaciones_list->StartRec - 1;
if ($notificaciones_list->Recordset && !$notificaciones_list->Recordset->EOF) {
	$notificaciones_list->Recordset->MoveFirst();
	$bSelectLimit = $notificaciones_list->UseSelectLimit;
	if (!$bSelectLimit && $notificaciones_list->StartRec > 1)
		$notificaciones_list->Recordset->Move($notificaciones_list->StartRec - 1);
...
while ($notificaciones_list->RecCnt < $notificaciones_list->StopRec) {
	...
	if (intval($notificaciones_list->RecCnt) >= intval($notificaciones_list->StartRec)) {
		$notificaciones_list->RowCnt++;

		// Set up key count
		$notificaciones_list->KeyCount = $notificaciones_list->RowIndex;

		// Init row class and style
		$notificaciones->ResetAttrs();
		$notificaciones->CssClass = "";
		if ($notificaciones->CurrentAction == "gridadd") {
		} else {
			$notificaciones_list->LoadRowValues($notificaciones_list->Recordset); // Load row values
		}
		$notificaciones->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$notificaciones->RowAttrs = array_merge($notificaciones->RowAttrs, array('data-rowindex'=>$notificaciones_list->RowCnt, 'id'=>'r' . $notificaciones_list->RowCnt . '_notificaciones', 'data-rowtype'=>$notificaciones->RowType));
...
// Close recordset
if ($notificaciones_list->Recordset)
...
<?php if ($notificaciones_list->Pager->RecordCount > 0 && $notificaciones_list->Pager->Visible) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($notificaciones_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $notificaciones_list->PageUrl() ?>start=<?php echo $notificaciones_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($notificaciones_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $notificaciones_list->PageUrl() ?>start=<?php echo $notificaciones_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($notificaciones_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $notificaciones_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($notificaciones_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $notificaciones_list->PageUrl() ?>start=<?php echo $notificaciones_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($notificaciones_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $notificaciones_list->PageUrl() ?>start=<?php echo $notificaciones_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
<?php if ($notificaciones_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $notificaciones_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $notificaciones_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $notificaciones_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($notificaciones_list->TotalRecs > 0 && (!$notificaciones_list->AutoHidePageSizeSelector || $notificaciones_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="notificaciones">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($notificaciones_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($notificaciones_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($notificaciones_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($notificaciones->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($notificaciones_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($notificaciones_list->TotalRecs == 0 && $notificaciones->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($notificaciones_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($notificaciones->Export == "") { ?>
<script type="text/javascript">
fnotificacioneslistsrch.FilterList = <?php echo $notificaciones_list->GetFilterList() ?>;
fnotificacioneslistsrch.Init();
fnotificacioneslist.Init();
</script>
<?php } ?>
<?php
$notificaciones_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($notificaciones->Export == "") { ?>
...
<?php
$notificaciones_list->Page_Terminate();
?>
